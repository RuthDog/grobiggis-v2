...
test("coordinates are nullable and internally range-validated", () => {
  assert.deepEqual(createInternalUserProfileInput({ firstName: null, locality: null, countryCode: "SE", latitude: null, longitude: null }), {
    firstName: null,
    locality: null,
    countryCode: "SE",
    latitude: null,
...
test("profile foundation does not touch growing, shopping or placement persistence code", () => {
  ...
  assert.doesNotMatch(`${page}\n${form}`, /localStorage|sessionStorage|indexedDB|navigator\.geolocation|geocoding|Open-Meteo|SMHI|Mapbox|Nominatim|Google Maps/i);
  assert.doesNotMatch(`${profileSchema}\n${migration}`, /street|address|postal|postcode|zip|house/i);
});
...
test("profile route, action and navigation are wired through auth-backed server flow", () => {
  ...
  assert.match(form, /saveUserProfileAction\(\{ firstName: name, locality: place \}\)/);
